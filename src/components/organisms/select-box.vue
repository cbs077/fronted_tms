<template>
  <div class="h-10 min-w-fit rounded border border-sk-gray focus:border-sk-red">
    <listbox
        ...
        >
          ...
          >
            ...
            >
              <li
                :class="[
                  active ? 'bg-sk-red bg-opacity-50 text-white' : 'text-gray-900',
                  'relative cursor-default select-none py-2 pl-10 pr-4',
                ]"
              >
                <span :class="[selected ? 'font-medium' : 'font-normal', 'block truncate']">{{ item.value }}</span>
                <span
                  v-if="selected"
                  class="absolute inset-y-0 left-0 flex items-center pl-3 text-amber-600"
                >
                  <CheckIcon
                    class="h-5 w-5 text-sk-red"
                    aria-hidden="true"
                  />
                </span>
              </li>
